...
管理画面はConvex接続時に `convex/admin.ts` の集計クエリを使います。`VITE_CONVEX_URL` が未設定の場合は、画面確認用としてlocalStorageの下書きデータを集計します。
...
- `src/components/ApplicationWorkspace.tsx` 申請フォーム、OCR下書き、チャット下書き入力
- `src/components/AdminDashboard.tsx` 管理画面
- `src/` フロントエンド
- `convex/schema.ts` Convex DBスキーマ
- `convex/applications.ts` 申請保存・検証・提出
- `convex/admin.ts` 管理画面向け集計・状態変更
